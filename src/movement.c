...
{
    cub3d->P->dir += direction;
    if (cub3d->P->dir < 0)
        cub3d->P->dir += 2 * M_PI;
    else if (cub3d->P->dir > 2 * M_PI)
        cub3d->P->dir -= 2 * M_PI;
    cub3d->P->dir_vec = angle_to_vec(cub3d->P->dir);
}
...
{
    t_vec new_dir1 = {.x = 0, .y = cub3d->P->dir_vec.y};
    t_vec new_dir2 = {.x = cub3d->P->dir_vec.x, .y = 0};
...
void    update_position(t_cub3d *cub3d, double angle)
{
    ...
    new_pos = vec_add(cub3d->P->pos, vec_scal_m(vec_rotate(cub3d->P->dir_vec, angle), PLAYER_SPEED));

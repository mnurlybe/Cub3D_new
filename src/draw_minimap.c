...
{
	for (size_t y = 0; y < cub3d->minimap->height; y++)
	{
		for (size_t x = 0; x < cub3d->minimap->width; x++)
		{
			...
			if (x % TILE_SIZE == 0 || y % TILE_SIZE == 0 || x == cub3d->minimap->width - 1 || y == cub3d->minimap->height - 1)
			{
